...
| 3 | `$4842-$5841` | Native one-byte font with style-selected Thin Pixel-7 English slots. Its `$5742-$5841` suffix is also the packed top-HUD atlas: `hud_font.py` replaces only decimal tiles `$5742-$5791`, while `$5792-$5841` (`A-F`, `Lv`, `Hp`, slash, meter, blanks) stays native and is audited read-only | `english_font.py` owns its English slots; `hud_font.py` exclusively owns `$5742-$5791` |
