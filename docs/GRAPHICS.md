...
| Rescue Team, warehouse, Bank Teller, and Blacksmith Info popup geometry | Exact-menu seven-interior-tile frames using six renderer-owned dynamic tiles; warehouse and Bank have stable `$B3` spill cells, Blacksmith stages the `Synthesis` suffix in `$B3`, clears its `$9C` Quit-cursor alias, and bank-syncs `$B9` blanks elsewhere, while Rescue exposes only `Password` overflow tiles `$A8/$BA`; active-VRAM-bank bottom border, staged-tile cleanup, and two-bank save/restore of the added ninth BG column | `service_menus.py` |
