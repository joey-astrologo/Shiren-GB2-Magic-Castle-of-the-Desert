...
REPAIR_ACTIVE_EDITOR_ADDRESS = 0x4240
...
INPUT_LOOP_BANK = 0x10
INPUT_LOOP_CALL_ADDRESS = 0x5AD2
INPUT_LOOP_CALL_EXPECTED = bytes.fromhex("CD7543")
INPUT_LOOP_TRAMPOLINE_ADDRESS = 0x7FF3
INPUT_LOOP_TRAMPOLINE_EXPECTED = bytes(12)
INPUT_LOOP_TRAMPOLINE_PATCH = bytes.fromhex(
    "3EF9214042CDAC09CD7543C9"
)
...
ENGLISH_NAVIGATION_SHA1 = "9471dcd6aae7a3735319ca9a9f7d3e6276ce6acf"
...
# Assembled from tools/rescue_presentation.asm at $F9:$4000-$4255. Keeping
...

fa95c1fe05d8fe09d0fa4ec1fef5c8fa95c14fc36041
""")
...
def owned_ranges():
    hooks = tuple(
        ...
        for bank, address, expected in (
            ...
            (INPUT_LOOP_BANK, INPUT_LOOP_CALL_ADDRESS, INPUT_LOOP_CALL_EXPECTED),
            (
                INPUT_LOOP_BANK,
                INPUT_LOOP_TRAMPOLINE_ADDRESS,
                INPUT_LOOP_TRAMPOLINE_EXPECTED,
            ),
        )
    )
...
def install(rom, verify_original=True, checksums=True):
    ...
    loop_call = _offset(INPUT_LOOP_BANK, INPUT_LOOP_CALL_ADDRESS)
    loop_call_patch = bytes(
        (
            0xCD,
            INPUT_LOOP_TRAMPOLINE_ADDRESS & 0xFF,
            INPUT_LOOP_TRAMPOLINE_ADDRESS >> 8,
        )
    )
    current = bytes(out[loop_call:loop_call + len(INPUT_LOOP_CALL_EXPECTED)])
    if verify_original and current not in (
        INPUT_LOOP_CALL_EXPECTED,
        loop_call_patch,
    ):
        raise RescuePresentationError(
            "graphical input loop changed at %s"
            % extract.location(INPUT_LOOP_BANK, INPUT_LOOP_CALL_ADDRESS)
        )
    out[loop_call:loop_call + len(loop_call_patch)] = loop_call_patch

    trampoline = _offset(INPUT_LOOP_BANK, INPUT_LOOP_TRAMPOLINE_ADDRESS)
    current = bytes(
        out[trampoline:trampoline + len(INPUT_LOOP_TRAMPOLINE_PATCH)]
    )
    if verify_original and current not in (
        INPUT_LOOP_TRAMPOLINE_EXPECTED,
        INPUT_LOOP_TRAMPOLINE_PATCH,
    ):
        raise RescuePresentationError(
            "graphical input trampoline reservation changed at %s"
            % extract.location(INPUT_LOOP_BANK, INPUT_LOOP_TRAMPOLINE_ADDRESS)
        )
    out[trampoline:trampoline + len(INPUT_LOOP_TRAMPOLINE_PATCH)] = (
        INPUT_LOOP_TRAMPOLINE_PATCH
    )
